...
def pour(state, jug1, jug2, jug_caps):
...
def get_successors(state, jug_caps):
    successors = []
    for jug1, jug2 in [(0, 1), (1, 0)]:
        new_state = pour(state, jug1, jug2, jug_caps)
        if new_state != state:
...
def a_star(start, goal, jug_caps):
    ...
    while open_list:
        ...
        for succ_state in get_successors(curr_state, jug_caps):
...
solution = a_star(start_state, goal_state, jug_caps)
if solution:
    print("Solution:")
